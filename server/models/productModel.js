...
class ProductModel {
	...
	// async getOneProducts(id, callback) {
	// 	await DB.query('SELECT * FROM products inner join structure on id_product=? and products.product_structure=structure.id_structure inner join product_category on products.id_product=product_category.id_product inner join category on product_category.id_category=category.id_category', [id], result => {
	// 		callback(result);
	// 	})
	// }
